...
test('builds a non-mutating patch release dry-run plan', () => {
  ...
  assert.ok(plan.docker.images.every(image => image.tags.includes(`${image.image}:0.1.2`)))
  assert.ok(plan.docker.images.every(image => image.tags.includes(`${image.image}:v0.1.2`)))
  ...
  assert.ok(plan.subscriptionAuth.note.includes('0.1.2'))
})
...
test('CLI renders human-readable and JSON dry-run output without leaking secrets', async () => {
  ...
  assert.equal(textPlan.targetVersion, '0.1.2')
