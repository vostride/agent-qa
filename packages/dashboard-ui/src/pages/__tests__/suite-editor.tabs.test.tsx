...
vi.mock('@/components/suite-navbar', () => ({ SuiteNavbar: () => <div /> }))
vi.mock('@/components/suite-visual-builder', () => ({ SuiteVisualBuilder: () => <div data-testid="builder" /> }))
...
describe('SuiteEditorPage tabs', () => {
  it('tab state is ephemeral — URL does not change when tab changes (no ?tab= written)', async () => {
    mount()
    await act(async () => { await new Promise((r) => setTimeout(r, 0)) })
    const builderTab = [...container.querySelectorAll('[role="tab"]')].find((el) => el.textContent?.includes('Builder'))
    const yamlTab = [...container.querySelectorAll('[role="tab"]')].find((el) => el.textContent?.includes('YAML'))
    expect(builderTab).not.toBeUndefined()
    expect(yamlTab).not.toBeUndefined()
