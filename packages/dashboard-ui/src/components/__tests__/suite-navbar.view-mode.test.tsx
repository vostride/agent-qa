...
const baseProps = {
  suiteName: 'Viewer Suite',
  suiteId: 's_view-123',
  unsaved: false,
  isCreateMode: false,
  mode: 'view' as const,
  suiteHref: '/suite/s_view-123/edit',
  isSaving: false,
  isValidating: false,
  isRunning: false,
...
describe('SuiteNavbar view mode contract', () => {
  it('renders only view-mode actions and viewer shortcut hints', () => {
    ...
    expect(container.textContent).not.toContain('s_view-123')
    ...
    expect(container.textContent).not.toContain('Run suite')
