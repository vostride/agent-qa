...
export function ExecutionTreeItem({ log, isSelected, onSelect }: ExecutionTreeItemProps) {
  ...
  return (
    <li role="treeitem" aria-level={2} aria-selected={isSelected} data-execution-id={log.id}>
      <button
        className={cn(
          'flex w-full items-start gap-2 py-1.5 px-3 text-left rounded-[2px] text-sm',
      ...
      >
        {log.status === 'passed' ? (
          <span className="h-1.5 w-1.5 rounded-full bg-emerald-500 shrink-0 mx-[5px]" />
        ) : (
          <span className="h-1.5 w-1.5 rounded-full bg-red-500 shrink-0 mx-[5px]" />
        )}
