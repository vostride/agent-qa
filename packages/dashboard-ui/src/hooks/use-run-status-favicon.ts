...
function getBadgeSvg(state: Exclude<RunFaviconState, 'default'>): string {
  if (state === 'failed') {
    return '<path d="M208 142L310 244M310 142L208 244" stroke="#EF4444" stroke-width="26" stroke-linecap="round"/>'
  }

  return `<circle cx="258" cy="192" r="61" fill="${BADGE_COLORS[state]}"/>`
}
...
export function getRunStatusFaviconHref(state: RunFaviconState): string {
  ...
  const svg = [
    '<svg width="324" height="257" viewBox="0 0 324 257" fill="none" xmlns="http://www.w3.org/2000/svg">',
