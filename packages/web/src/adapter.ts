...
import { convertKeysForPlatform, isMacPlatform, warnIfOutOfBounds } from '@vostride/agent-qa-core'
...
import { formatMissingBrowserError, isPlaywrightMissingBrowserError } from './browser-install-error.js'
...
export class WebPlatformAdapter implements PlatformAdapter {
  ...
  async setup(config: PlatformConfig): Promise<void> {
    ...
    const browserName = config.browser?.name ?? 'chromium'
    const launcher = launchers[browserName]

    try {
      this.browser = await launcher.launch({
        headless: config.browser?.headless ?? true,
        ...(browserName === 'chromium' ? { args: ['--disable-blink-features=AutomationControlled'] } : {}),
      })
    } catch (error) {
      if (isPlaywrightMissingBrowserError(error)) {
        throw new Error(formatMissingBrowserError(browserName))
      }
      throw error
    }
    ...
    // Clipboard write permission — chromium only (firefox/webkit reject this permission per Pitfall 3)
    ...
    // Hide automation signals so bot-protected sites don't redirect
  ...
  get isBrowserDisconnected(): boolean {
    return this.disconnected
  }

  async observe(options?: ObserveOptions): Promise<ScreenState> {
    if (!this.page) {
      throw new Error('WebPlatformAdapter not initialized — call setup() first')
    }
  ...
  async execute(action: Action): Promise<ActionResult> {
    ...
    try {
      ...
      try {
        screenshot = await this.page.screenshot({ scale: 'css' })
      } catch {
  ...
  async screenshot(): Promise<Buffer | undefined> {
    try {
      if (this.page) {
        return await this.page.screenshot({ scale: 'css' })
      }
      return undefined
    } catch {
      return undefined
  ...
  private async executeAction(action: Action): Promise<ActionResult> {
    ...
    // Validate action-element compatibility for ref-targeting actions
    if ('ref' in action && typeof (action as any).ref === 'string') {
    ...
    switch (action.type) {
      ...
      case 'waitFor': {
        // Wait for condition: try as text content first, then fall back to selector.
        // The LLM generates natural language conditions ("loading spinner disappears")
        // which aren't CSS selectors — use page.waitForFunction for text matching.
        const timeout = action.timeout ?? 5000
        try {
          // Runs in browser context — checks if the condition text appears on page
          await page.waitForFunction(
            `document.body && document.body.innerText.includes(${JSON.stringify(action.condition)})`,
            { timeout },
          )
        } catch {
          // Fall back to waitForSelector for CSS-selector-shaped conditions
          try {
            await page.waitForSelector(action.condition, { timeout })
          } catch {
            // Best-effort — return success so the agent loop can re-observe
          }
        }
        return { success: true }
      }
      ...
      // Mobile-only actions — graceful unsupported messages
      case 'tap':
      case 'swipe':
      case 'longpress':
      case 'pinch':
      ...
      case 'tapCoordinate': {
        ...
        if (vp) {
          warnIfOutOfBounds({ x: action.x, y: action.y }, { width: vp.width, height: vp.height }, 'tapCoordinate')
        }
      ...
      case 'executeScript': {
        return { success: false, error: 'executeScript is a mobile-only action' }
      }
